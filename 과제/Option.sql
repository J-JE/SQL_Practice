...
--3.
--주소지가 강원도나 경기도인 학생들 중 1900년대 학번을 가진 학생들의 이름과 학번, 주소의 이름의
--오름차순으로 화면에 출력하시오. 단, 출력헤더에는 "학생이름", "학번", "거주지 주소" 가 출력되도록 한다.
...
SELECT A.CLASS_NAME, C.PROFESSOR_NAME
FROM TB_CLASS A
JOIN TB_CLASS_PROFESSOR B USING(CLASS_NO)
LEFT JOIN TB_PROFESSOR C USING(PROFESSOR_NO);
...
FROM TB_STUDENT A
JOIN TB_GRADE B USING(STUDENT_NO)
...
--12.
--2007년도에 '인간관계론' 과목을 수강한 학생을 찾아 학생이름과 수강학기를 표히사는 SQL 문장을 작성하시오.
...
SELECT CLASS_NAME, DEPARTMENT_NAME
FROM TB_CLASS A
LEFT JOIN TB_DEPARTMENT B USING(DEPARTMENT_NO)
WHERE CATEGORY = '예체능'
AND A.CLASS_NO NOT IN (SELECT CLASS_NO FROM TB_CLASS_PROFESSOR);

--14.
--춘 기술대학교 서반아어학과 학생들의 지도교수를 게시하고자 한다. 학생이름과 지도교수 이름을 찾고 만일 지도교수가 없는 학생일 경우
...
--15.
--휴학생이 아닌 학생 중 평점이 4.0 이상인 학생을 찾아 그 학생의 학번, 이름, 학과이름, 평점을 출력하는 SQL 문을 작성하시오.
SELECT A.STUDENT_NO "학번",A.STUDENT_NAME "이름",C.DEPARTMENT_NAME "학과이름", TRUNC(AVG(B.POINT),8) 평점
FROM TB_STUDENT A
JOIN TB_GRADE B ON A.STUDENT_NO=B.STUDENT_NO
JOIN TB_DEPARTMENT C ON A.DEPARTMENT_NO=C.DEPARTMENT_NO
WHERE A.ABSENCE_YN = 'N'
GROUP BY A.STUDENT_NO, A.STUDENT_NAME, C.DEPARTMENT_NAME
HAVING AVG(B.POINT)>=4
ORDER BY 1;

--16.
--환경조경학과 전공과목들의 과목 별 평점을 파악할 수 있는 SQL 문을 작성하시오.
SELECT B.CLASS_NO, B.CLASS_NAME, TRUNC(AVG(C.POINT),8)
FROM TB_DEPARTMENT A
JOIN TB_CLASS B ON A.DEPARTMENT_NO=B.DEPARTMENT_NO
JOIN TB_GRADE C ON B.CLASS_NO=C.CLASS_NO
WHERE A.DEPARTMENT_NAME='환경조경학과'
...
FROM (SELECT A.STUDENT_NO, A.STUDENT_NAME, AVG(C.POINT)
      FROM TB_STUDENT A
...
SELECT A.DEPARTMENT_NAME "계열 학과명", ROUND(AVG(POINT),1) "전공평점"
FROM TB_DEPARTMENT A
JOIN TB_CLASS B ON A.DEPARTMENT_NO=B.DEPARTMENT_NO
JOIN TB_GRADE C ON B.CLASS_NO=C.CLASS_NO
WHERE A.CATEGORY =(SELECT CATEGORY FROM TB_DEPARTMENT WHERE DEPARTMENT_NAME ='환경조경학과')
AND B.CLASS_TYPE LIKE '전공%'
